...
bool BranchED::AlignData(AlignBy alignType, bool waitToComplete ) /* Perform data alignment, parameter: Time, Volts, All (Dflt), PrbsVolts, PrbsAll */
{
    SendCommand("AlignData %s\n",AlignBy_Strings[(int)alignType] );

    bool retn=true;
    if( waitToComplete )
    	retn=WaitForAlignmentToComplete();
...
bool BranchED::WaitForAlignmentToComplete()
{
	...
	return !strncasecmp( buffer, SUCCESS_PREFIX, strlen(SUCCESS_PREFIX) );
